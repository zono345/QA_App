...
class QuestionsListAdapter(context: Context): BaseAdapter() {
    ...
    override fun getView(posision: Int, view: View?, parent: ViewGroup): View {
        ...
        titleText.text = mQuestionArrayList[posision].title

        val nameText = convertView.nameTextView as TextView
        nameText.text = mQuestionArrayList[posision].name

        val resText = convertView.resTextView as TextView
        val resNum = mQuestionArrayList[posision].answers.size
        resText.text = resNum.toString()

        val bytes = mQuestionArrayList[posision].imageBytes
        if (bytes.isNotEmpty()) {
